...
wk.cumul.sum<-ddply(subset(df.cumul.sum,Rank!="Low"), c("Week","Species"), summarise, 
...
wk.cum.by.species = cast(wk.cumul.sum, Week~Species,value="Mean.Wk.Prop") 
...
#### mixed model - channel catfish test case
df.cat<-subset(df.high,fSpecies=="Channel Catfish")
m1 <- glmer(cumul.sum ~ Week + (1 | fYear), 
            data = df.cat, 
            family = binomial)
summary(m1) ## model does not converge..
new.cat <- expand.grid(Week = unique(df.cat$Week),
                        fYear = unique(df.cat$fYear))

new.cat$predicted_prob <- predict(m1, new.cat, type = "response", re.form = NA) # use re.form = NULL if including random effects
...
p <- p +  geom_vline(xintercept=17.5, linetype="dotdash",colour="red", size=2)
p <- p +  geom_vline(xintercept=28.5, linetype="dotdash",colour="red", size=2)
p <- p + geom_jitter(data = df.cat, 
                     aes(x = Week, y = cumul.sum), 
                     width = 0.2, height = 0.02, alpha = 0.3)
p <- p + geom_line(data=new.cat, 
                   aes(x = Week, y = predicted_prob),colour="black",linewidth=2)
p <- p + geom_smooth(data=df.cat,aes(x=Week,y=cumul.sum))
p 
