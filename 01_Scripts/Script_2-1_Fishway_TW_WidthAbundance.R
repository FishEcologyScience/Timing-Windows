...
df.d<-aggregate(Quantity~Species+Year+Day+YMD,data=df.3,FUN=sum) ## summarize catch by day-year

df.a<-merge(df.d,sum.species,by=c("Species"))
...
#
df.d.temp<-merge(df.day.base,df.d,by=c("Year","Day","Species","YMD"),all=T) ## combine dayly catch
...
#
#df.a<-merge(df.d.plot,sum.species,by=c("Species"))
...
# dayly means - problematic b/c of unequal sampling among years
df.d.plot.mean<-aggregate(Prop.Year~Species+Day,data=df.d.plot,FUN=mean)
df.d.plot.mean$fSpecies<-as.factor(df.d.plot.mean$Species)
df.d.plot.mean.2<-merge(df.d.plot.mean,sum.species,by="Species",all=T)

df.d.plot.mean.2$fSpecies2 <- factor(df.d.plot.mean.2$fSpecies, levels = c("Rainbow Trout", "Northern Pike", "White Sucker", "Yellow Perch", "Rudd", "Brown Bullhead",
...
# subset(df.wk.plot.mean.2,Rank=="High")
# subset(df.wk.plot.mean.2,Rank!="Low")

conf <- 0.05 #or whatever
quantile <- qnorm(1-conf/2)
upper_bound <- my_mean + quantile*my_std_dev
lower_bound <- my_mean - quantile*my_std_dev
...
l.model <- lm(Day.Sum ~ 1, subset(df.b,Species=="Common Carp"))
